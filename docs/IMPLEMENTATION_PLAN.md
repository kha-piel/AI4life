...
- Complete label reading is the only product path.
- The user may select diabetes screening before opening the camera.
...
2. Complete-label mode traverses health-profile selection → camera/library queue
   → API → validated response → Vietnamese speech.
...
- Diabetes output is screening only. A positive model claim without visible serving
  size and total carbohydrate is deterministically replaced with `uncertain`.
